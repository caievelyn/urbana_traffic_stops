...
x <- left_join(df, ages, by = "incident_number") %>%
  rename("age_at_contact" = "age_at_contact.y") %>%
  mutate(age_at_contact_new = case_when(is.na(age_at_contact.x) ~ age_at_contact,
                                    is.na(age_at_contact) ~ NA_real_,
...
```{r reshaping, echo=FALSE, eval=FALSE}
...
tstops_demo <- tstopoffenses %>%
  ...
  select(date_of_contact, time_of_contact, incident_number,
        ...
        type_of_violation, vehicle_year, crime_description, crime_status_description, crime_category_description) %>%
...
# Fit regression model to likelihood of being arrested based off of race, age, gender

# Construct simulation that demonstrates the likelihood of being stopped vs not being stopped:
# 1s correspond to everyone in the dataset who was stopped (by incident number)
# 0s correspond to everyone else of drivable age?
