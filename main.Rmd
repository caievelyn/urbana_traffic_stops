...
disparity_ratios <- tstops_demo %>%
  ...
  mutate(race_ct = n(),
         ...
         disparity_ratio = case_when(
           race_description == "AMERICAN INDIAN/ALASKAN" ~ race_percentage / 0.24,
           race_description == "ASIAN" ~ race_percentage / 8.37,
           race_description == "BLACK" ~ race_percentage / 22.26,
           race_description == "HISPANIC" ~ race_percentage / 4.84,
           race_description == "WHITE" ~ race_percentage / 62.23)) %>%
  ungroup() %>%
  select(year, race_description, disparity_ratio) %>%
  unique()
...
# Gender
disparity_ratios_gender <- tstops_demo %>%
  filter(year %in% c("2018", "2019", "2020")) %>%
  group_by(year) %>%
  mutate(all = n()) %>%
  ungroup() %>%
  ...
  mutate(race_ct = n(),
         ...
         disparity_ratio = case_when(
           ...
           race_description == "WHITE" ~ race_percentage / 62.23)) %>%
  ungroup() %>%
  select(year, race_description, disparity_ratio, sex_description) %>%
  unique()

disparity_ratios_gender %>%
  ggplot(aes(year, disparity_ratio, color = race_description)) +
  geom_line() +
  scale_x_continuous(breaks = c(2018, 2019, 2020)) +
  facet_grid(~sex_description)
...
1. My numbers are off, what could be the case? The biggest disparity is 2018, everything else is 1 or 2 stops off
